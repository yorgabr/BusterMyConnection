...
Begin by cloning the project and ensuring you are on Windows PowerShell 5.1, which is the supported baseline. From the repository root, install the three modules the build depends upon — `InvokeBuild`, `Pester` (6.0 or newer; the project moved off the 5.x line, which is now in maintenance mode), and `PSScriptAnalyzer` — into your user scope. With those present, a single invocation of `Invoke-Build` runs the full quality gate: it lints the source, executes every test, and measures code coverage against the project's 60% floor.
