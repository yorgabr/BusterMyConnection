...
function Test-PowerShellCompatibility {
    ...
    if ($PSVersionTable.PSVersion.Major -lt 5 -or
        ...
        $issues += "Requires PowerShell 5.1 or later. Current: $($PSVersionTable.PSVersion)"
    }

    if (-not (Get-Command 'Invoke-WebRequest' -ErrorAction SilentlyContinue)) {
        $issues += "Invoke-WebRequest cmdlet not available"
    }

    if ($issues) {
        throw "Compatibility issues detected: $($issues -join '; ')"
    }
}
...
function Test-UpstreamProxyConnectivity {
    ...
    try {
        ...
        Out-Warn "Socket error connecting to upstream proxy ${proxyHost}:${proxyPort}: $($_.Exception.SocketErrorCode)"
        return $false
    }
    catch {
...
#==============================
# VPN Detection System (PS 5.1 Compatible)
#==============================

function New-VpnDetectionResult {
    param(
        [bool]$IsVpnActive = $false,
        [string]$VpnType = "None",
        [string]$Description = "No VPN detected",
        [scriptblock]$ReconcileAction = $null
    )
    ...
    return [PSCustomObject]@{
        ...
        PSTypeName = 'VpnDetectionResult'
    }
}

function Test-BigIpVpn {
    param([string]$IniPath)

    $regPath = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings'
    try {
        $autoConfigUrl = (Get-ItemProperty -Path $regPath -Name AutoConfigURL -ErrorAction Stop).AutoConfigURL
        $bigIpPacRegex = '^http://127\.0\.0\.1:(44[7-9]\d{2}|45[0-7]\d{2})/'

        if ($autoConfigUrl -and $autoConfigUrl -match $bigIpPacRegex) {
            $reconcileAction = {
                param([string]$Path)
                Set-CntlmParentProxyFromSystem -IniPath $Path
            }

            return New-VpnDetectionResult `
                -IsVpnActive $true `
                -VpnType "BIG-IP Edge Client" `
                -Description "Local PAC server detected at $autoConfigUrl" `
                -ReconcileAction $reconcileAction
        }
    } catch {}

    return $null  # Continue to next detector
}

function Test-CiscoAnyConnect {
    param([string]$IniPath)
    ...
    if ($isRunning -or $isInstalled) {
        $reconcileAction = { param([string]$Path) Set-CntlmParentProxyFromSystem -IniPath $Path }
        return New-VpnDetectionResult -IsVpnActive $true -VpnType "Cisco AnyConnect" `
            -Description "Cisco VPN client detected (running: $(if($isRunning){'yes'}else{'no'}))" `
            -ReconcileAction $reconcileAction
    }
...
function Invoke-VpnDetectionChain {
    param([string]$IniPath)

    # Chain of responsibility pattern using functions instead of classes
    $detectors = @(
        { param($p) Test-BigIpVpn -IniPath $p }
        { param($p) Test-CiscoAnyConnect -IniPath $p }
    )

    foreach ($detector in $detectors) {
        $result = & $detector -IniPath $IniPath
        if ($result -ne $null) { return $result }
    }

    return New-VpnDetectionResult  # Default: no VPN
}

#==============================
# END VPN Detection System
#==============================
...
function New-CntlmConfiguration {
    ...
    Set-Content -LiteralPath $OutputPath -Value $configContent -Encoding UTF8

    # Clear password from memory if used
    if ($plainPassword) {
        $plainPassword = $null
        [GC]::Collect()
    }
...
function Set-CntlmParentProxyFromSystem {
    ...
    if ($updatedContent -ne $originalContent) {
        ...
        Set-Content -LiteralPath $IniPath -Value $updatedContent -Encoding UTF8
...
function Test-WgetSansProxyBehavior {
    <#
    .SYNOPSIS
        Performs a diagnostic test to verify direct internet connectivity.

    .DESCRIPTION
        This function attempts a web request without explicitly specifying a proxy,
        which helps verify whether the system's default proxy configuration is
        functioning correctly. It serves as a sanity check that CNTLM is actually
        needed and that the network path to the internet is viable. This test is
        only available in Windows PowerShell where WinINET integration exists.
    #>
    ...
    try {
        ...
        Invoke-WebRequest -Uri $testUrl -OutFile $null -MaximumRedirection 5 -UseBasicParsing
        Out-Success "Direct request succeeded—CNTLM may not be necessary for this network."
    }
    catch {
        $errMsg = $_.Exception.Message
        Out-Warn "Direct request failed as expected behind corporate proxy: $errMsg"
    }
}
#------------------------------
# Main execution flow
...
if (Test-Path -LiteralPath $IniPath) {
    ...
    # PS 5.1 compatible $PSScriptRoot handling
    $scriptRoot = if ($PSScriptRoot) { $PSScriptRoot } else { $PWD }
    $fallbackIni = Join-Path -Path $scriptRoot -ChildPath 'cntlm.ini'
...
$vpnResult = Invoke-VpnDetectionChain -IniPath $resolvedIni
...
try {
    ...
    # PS 5.1 compatible: Wait for port to be listening, not just process existence
    $maxWait = 10
    $waited = 0
    $cntlmStarted = $false
    ...
    while ($waited -lt $maxWait) {
        ...
        $listener = Get-NetTCPConnection -LocalPort 3128 -State Listen -ErrorAction SilentlyContinue
        if ($listener) {
            $cntlmStarted = $true
            break
        }
    ...
    if ($runningProcesses -and $cntlmStarted) {
        ...
        Out-Error "CNTLM process did not persist after startup or port is not listening. Review log file for details: ${LogFile}"
        exit 1
    }
}
catch {
    $errorMessage = $_.Exception.Message
    $fmt = 'Failed to start CNTLM: {0}'
    Out-Error ($fmt -f $errorMessage)
    exit 1
}